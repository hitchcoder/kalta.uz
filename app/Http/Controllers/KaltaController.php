...
class KaltaController extends Controller
{
    ...
    public function show(Kalta $kalta)
    {
        ...
        if (!$kaltaable) {
            Log::warning('Kalta has no associated kaltaable resource.', ['kalta_id' => $kalta->id]);
            abort(404);
        }
        ...
        if (!empty($kaltaable->path)) {
            if (!Storage::disk('public')->exists($kaltaable->path)) {
                Log::error('File missing from storage for kalta.', ['kalta_id' => $kalta->id, 'path' => $kaltaable->path]);
                abort(404);
            }
        ...
        Log::error('Unrecognized kaltaable type for kalta.', ['kalta_id' => $kalta->id, 'type' => get_class($kaltaable)]);
