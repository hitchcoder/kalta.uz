...
class KaltaController extends Controller
{
    ...
    public function show(Kalta $kalta)
    {
        ...
        if ($kaltaable === null) {
            Log::warning("Kalta {$kalta->url} has no associated resource.");
        ...
        if (!empty($kaltaable->path)) {
            if (!Storage::disk('public')->exists($kaltaable->path)) {
                Log::warning("File missing on disk for kalta {$kalta->url}: {$kaltaable->path}");
                throw new NotFoundHttpException();
            }

            return Storage::disk('public')->download($kaltaable->path, $kaltaable->name);
        }

        Log::warning("Unhandled kaltaable type for kalta {$kalta->url}: " . get_class($kaltaable));
        throw new NotFoundHttpException();
    }
