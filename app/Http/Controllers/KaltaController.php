...
class KaltaController extends Controller
{
    ...
    public function show(Kalta $kalta)
    {
        ...
        if (! $kaltaable) {
            Log::warning('Kalta has no associated resource.', ['kalta_id' => $kalta->id, 'url' => $kalta->url]);
            abort(404);
        }

        if ($kaltaable instanceof Short) {
            return redirect()->away($kaltaable->long_url);
        }

        if ($kaltaable instanceof File) {
            if (empty($kaltaable->path) || ! Storage::disk('public')->exists($kaltaable->path)) {
                Log::warning('Requested file is missing from storage.', ['kalta_id' => $kalta->id, 'path' => $kaltaable->path ?? null]);
                abort(404);
            }
        ...
        Log::warning('Unknown kaltaable type encountered.', ['kalta_id' => $kalta->id, 'type' => get_class($kaltaable)]);
        abort(404);
    }
