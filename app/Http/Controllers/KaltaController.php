...
class KaltaController extends Controller
{
    ...
    public function show(Kalta $kalta)
    {
        ...
        if (!$kaltaable) {
            Log::warning("Kalta {$kalta->url} has no related resource.");
            abort(404);
        }
        ...
        if ($kaltaable instanceof File) {
            ...
            Log::error("File missing on disk for kalta {$kalta->url}.", ['path' => $kaltaable->path]);
            abort(404);
        }

        Log::error("Unknown kaltaable type for kalta {$kalta->url}.", ['type' => get_class($kaltaable)]);
        abort(404);
    }
